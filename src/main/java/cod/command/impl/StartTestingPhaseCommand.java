...
public final class StartTestingPhaseCommand implements ICommand {
    private final String username;
    private final ObjectMapper mapper = new ObjectMapper();

    public StartTestingPhaseCommand(final JsonNode args) {
    ...
    public ObjectNode execute() {
        ...
        if (!"MANAGER".equals(db.getUser(username).getRole())) {
            result.put("status", "error");
            result.put("message", "The user does not have permission to execute this command: "
                    + "required role MANAGER; user role " + db.getUser(username).getRole() + ".");
            return result;
        }

        boolean hasActiveMilestones = db.getMilestones().stream().anyMatch(m ->
                m.getTickets().stream().map(id -> db.getTickets().get(id))
                        .anyMatch(t -> !"CLOSED".equals(t.getStatus())
